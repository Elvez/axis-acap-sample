...
int main() {
    ...
    // 3. Start stream
    if (!vdo_stream_start(g_list_first(streams), &error)) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        VdoBuffer* buffer = vdo_stream_get_buffer(g_list_first(streams), &error);
        ...
        if (!vdo_stream_buffer_unref(g_list_first(streams), &buffer, &error)) {
